...
void UdivPlugin::OnExecute(lc3_state& state, lc3_state_change& changes)
{
    ...
    lc3_change_info change_reg0 = { true, 0, (unsigned short)state.regs[0] };
    changes.info.push_back(change_reg0);
    lc3_change_info change_reg1 = { true, 1, (unsigned short)state.regs[1] };
    changes.info.push_back(change_reg1);
